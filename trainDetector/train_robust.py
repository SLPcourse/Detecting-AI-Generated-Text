...
class ModelTrainer:
    def __init__(
        ...
        # if load pre-trained parameters
        if load_model_path:
    ...
    def run_training(self):
        ...
        train_loader = DataLoader(
            ...
            shuffle=True,
            collate_fn=self.collate_fn, # modification on each batch
        )
...
if __name__ == "__main__":
    for recipe in ['textfooler', 'textbugger', 'bae', 'deepwordbug', 'pwws']:
        trainer = ModelTrainer(
            train_file = f"../dataset/train_{recipe}_1000.json",
            val_file = f"../dataset/val_{recipe}_100.json",
            test_file = f"../dataset/test_{recipe}_200.json",
            load_model_path = "models/best_model_v1.pt",
            save_path = f"models/adv_{recipe}_model.pt",
            num_epochs=10,
        )
